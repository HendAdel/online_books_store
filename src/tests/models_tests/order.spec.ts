...
describe("order Model", () => {
    ...
    it('showById method should return one order with the same id', async () => {
    ...
    it('updateById method should return one order with new data', async () => {
   ...
   it('create method should add new order details', async () => {
        const result = await orderM.create_o_d(parseInt(order.id as unknown as string), 1, 1);
    ...
    it('deleteById method should remove one order with the same id', async () => {
        await orderM.deleteById('1');
        const result = await orderM.index();
        expect(result.length).toBeGreaterThan(0);
    });
